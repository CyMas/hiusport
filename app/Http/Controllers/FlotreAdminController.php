...
class FlotreAdminController extends Controller
{
    ...
    {   
		...
		return view('admin.home')->with('users', $users)->with('pesanans', $pesanans)->with('contacts', $contacts)->with('produks',$produks);
    }
    ...
    public function listpesananreport()
    {
        $no = 1;
        $pdf = DB::select('select *from pesanans');
        $jumlah = DB::select('select sum(total_harga) as jumlah from pesanans where status=?', ['Tuntas']);
        $html = view('admin.pesanan.listpesananreport')->with('pdf',$pdf)->with('jumlah',$jumlah)->with('no',$no);
